...
public class VMultiItem extends VItem {
	...
	public void registerIcons(IconRegister par1) {
		this.icons = new Icon[128];
		for (int i = 0; i < 128; ++i) {
			if (i >= this.items) {
				this.icons[i] = par1.registerIcon("diverse:missingetexture");
			}
			this.icons[i] = par1.registerIcon("diverse:" + this.texture + "_" + i);
		}
	}
	...
	public String getUnlocalizedName(ItemStack e) {
		int i = MathHelper.clamp_int(e.getItemDamage(), 0, 127);
		if (e.getItemDamage() >= this.items) {
			return "diverse.unknown";
		}
		return "item.diverse." + this.texture + "." + i;
	}
}
