...
import by.fxg.diverse.common.tile.VTileInventory;
...
public class VBlockGuiBase extends VBlockBase {
	...
	public VBlockGuiBase install(String unlocalizedName) {
		this.setUnlocalizedName("vgui." + unlocalizedName);
		return this;
	}
	...
	public boolean onBlockActivated(World par1World, int x, int y, int z, EntityPlayer player, int par6, float par7, float par8, float par9) {
		if (par1World.isRemote) {
			...
			if (var10 != null) {
				player.openGui(Viod.instance, this.guid, par1World, x, y, z);
			}
			return true;
		} else {
			return false;
	...
	public TileEntity createTileEntity(World world, int metadata) {
		if (!this.tilename.equals("")) {
			try {
				Class clazz = Class.forName("by.fxg.diverse.common.tile." + this.tilename);
